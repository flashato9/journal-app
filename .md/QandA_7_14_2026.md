...
## What's the difference between `version` and `android.versionCode`?

`version` maps to Android's `versionName` — a cosmetic human-readable string, never compared by the OS. `android.versionCode` is an integer the package manager actually uses to allow/block installs (it refuses a lower value than what's already installed); bumping `version` alone does nothing to satisfy that check.
...
## How do I check which devices adb currently sees?

`adb devices` lists them, or `adb devices -l` adds model/product detail. A status of `device` means it's authorized and ready; `unauthorized` means you still need to accept the USB-debugging prompt on the phone.
...
`journal.db` (SQLite, all metadata incl. `mediaUri`/`profileImagePath` strings) lives in the app's private document directory. Actual memory photo/video bytes go to `<Documents>/images/memories/` in dev builds or the OS Photo Library in production; sound recordings always stay app-private in `<Documents>/sounds/memories/` (`services/mediaStorage.ts`). Login credentials/tokens live in the OS secure keychain via `expo-secure-store`, not a plain file.
...
## Is Zustand for making database calls better?

No — Zustand is purely client-side in-memory state (like Context/`useState`), unrelated to fetching/caching DB or API data. That job belongs to something like TanStack Query or SWR, which is often paired _with_ Zustand (Query owns server/DB data, Zustand owns UI state).

# components/memories/MediaGallery/MediaCard.tsx

## Could the video tile show an animated GIF instead of a frozen first frame?

Technically yes, but Expo has no built-in video→GIF API, so it would need a heavy native module (e.g. `ffmpeg-kit-react-native`) to extract frames and encode a GIF on every video added, plus a second generated asset to store and clean up. A play-icon overlay on the existing frozen frame gives the same "this is playable" signal at near-zero cost and matches how Photos/WhatsApp/Instagram handle video thumbnails.
